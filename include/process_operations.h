...
class process_operations {
 ...
 private:
  ...
  static void execute_kill_process(const int process_pid,
    ...
    memset(&_process, 0,
           sizeof(_process));  // zero out the allocated proc_info memory
  ...
  static void execute_kill_process_optimized(const int process_pid,
    ...
    if (string_utils::search_substring(process_fake_name, process_name)) {
      ...
      if (it_process != map_processes.end()) {
        ...
        if (NULL != hProcess) {
          ...
          if (res == 0) {
            ucout << _T("Process '") << it_process->second.procName
                  << _T("' PID [") << it_process->second.procPID
        ...
        ucout << _T("Process '") << it_process->second.procName << _T("' PID [")
              << process_pid << _T("] was terminated.") << std::endl;
      }
